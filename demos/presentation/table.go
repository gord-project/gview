...
const tableBasic = `[green]func[white] [yellow]main[white]() {
    table := gview.[yellow]NewTable[white]().
...
func Table(nextSlide func()) (title string, content gview.Primitive) {
	table := gview.NewTable().
		SetFixed(1, 1)
	for row, line := range strings.Split(tableData, "\n") {
		for column, cell := range strings.Split(line, "|") {
			...
			tableCell := gview.NewTableCell(cell).
				SetTextColor(color).
				SetAlign(align).
	...
	separator := func() {
		table.SetBorders(false).
			SetSelectable(false, false).
			SetSeparator(gview.Borders.Vertical)
		code.Clear()
